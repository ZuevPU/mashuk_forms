...
(function () {
  ...
  function pin() {
    ...
    if (!(lastH && Math.abs(h - lastH) < 8)) {
      lastH = h;
      var px = h + "px";
      clip.style.cssText = "box-sizing:border-box;width:100%;max-width:100%;height:" + px + ";max-height:" + px + ";min-height:" + px + ";overflow:hidden;background:#fafafa;line-height:normal;margin:0 auto;padding:0;";
      frame.style.cssText = "display:block;width:100%;max-width:100%;height:" + px + ";max-height:" + px + ";border:0;background:#fafafa;pointer-events:auto;touch-action:manipulation;margin:0;padding:0;";
      frame.setAttribute("scrolling", "yes");
    }
    busy = 0;
  }
...
def block(kind):
    if kind == "apply":
        ...
        clip, frame = "mshk-m-clip", "mshk-m-frame"
        src = "https://zuevpu-mashuk-forms-e759.twc1.net/muni"
        title = "Mashuk municipal apply"
        old = "mshk-muni-wrap"
        page = "municipal application"
    else:
    ...
    if contained:
        comment = (
            "<!-- Tilda T123. Page: " + page + ".\n"
            "     DELETE the old iframe block, then paste this.\n"
            "     Record: do NOT stretch. Keep standard page width.\n"
            "     Height: 0 / Auto. Zero extra padding. -->\n"
        )
        wrap = (
            '<div id="' + clip + '" style="box-sizing:border-box;width:100%;max-width:1200px;'
            'height:100vh;overflow:hidden;background:#fafafa;margin:0 auto;padding:0;">\n'
            '  <iframe id="' + frame + '" src="' + src + '" title="' + title + '" '
            'loading="eager" scrolling="yes" allow="clipboard-write" '
            'style="display:block;width:100%;max-width:100%;height:100vh;border:0;'
    ...
    return comment + wrap + "<script>" + js + "</script>\n"
...
def main():
    ...
    (TILDA / "tilda-iframe-muni.html").write_text(muni_html, encoding="utf-8")
    print("wrote", TILDA / "tilda-iframe-block.html", len(apply_html))
    print("wrote", TILDA / "tilda-iframe-info.html", len(info_html))
    print("wrote", TILDA / "tilda-iframe-muni.html", len(muni_html))
